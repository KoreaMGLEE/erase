...
### T5-XL × MNLI (k=30%, 3 epochs, Adafactor, 3 splits 평균) — 완료

| Condition | Dev | HANS | ANLI-r1 | ANLI-r2 | ANLI-r3 | ANLI avg | OOD avg |
|-----------|------|------|---------|---------|---------|----------|---------|
| C1 Full | 0.9011 | 0.6984 | 0.5220 | 0.3910 | 0.3967 | 0.4366 | 0.5020 |
| C2 Random | 0.9019 | 0.7031 | 0.5183 | 0.3930 | 0.3928 | 0.4347 | 0.5018 |
| C3 Self-easy | 0.9017 | 0.7006 | 0.5250 | 0.3783 | 0.3850 | 0.4294 | 0.4972 |
| C4 Intersect | 0.9037 | 0.6704 | 0.5253 | 0.3897 | 0.4064 | 0.4405 | 0.4980 |
| **C5 Union** | 0.9016 | **0.7051** | **0.5430** | 0.3893 | 0.4014 | **0.4446** | **0.5097** |
| C6 Dedup | 0.9004 | 0.6792 | 0.5140 | 0.3757 | 0.4008 | 0.4302 | 0.4924 |

**T5-XL 관찰**:
1. **C5 Union이 OOD 전체 평균에서 최고** (0.5097), HANS와 ANLI-r1 모두 최고
2. Dev는 모든 조건에서 ~0.90으로 동일 → 필터링이 broad performance에 영향 없음
3. C3 Self-easy는 오히려 baseline보다 OOD 낮음 → 자기 자신의 easy 제거는 비효과적
4. C6 Dedup이 기대만큼 좋지 않음 → dedup 전략 재검토 필요

### BERT-large × MNLI (k=30%, 3 epochs, lr=3e-5, 3 splits 평균) — 진행 중

일부 완료된 결과 (11/18, C2/C3/C6_split1 미완):
...
### BERT-large × MNLI k=5% (3 epochs, lr=3e-5, 3 splits 평균) — 완료

실패 run 2개 제외 (C2_random/split2, C4_intersect/split2 — 학습 불안정, 재실행 중)

| Condition | N | Dev | HANS | ANLI-r1 | ANLI-r2 | ANLI-r3 | ANLI avg | OOD avg |
|-----------|---|------|------|---------|---------|---------|----------|---------|
| C1 Full | 3 | 0.8062 | 0.5473 | 0.2297 | 0.2787 | 0.2983 | 0.2689 | 0.3385 |
| C2 Random | 2 | 0.8064 | 0.5315 | 0.2250 | 0.2775 | 0.3025 | 0.2683 | 0.3341 |
| C3 Self-easy | 3 | 0.8055 | 0.5219 | 0.2287 | 0.2763 | 0.3000 | 0.2683 | 0.3317 |
| C4 Intersect | 2 | 0.8045 | 0.5171 | 0.2350 | 0.2905 | 0.3050 | 0.2768 | 0.3369 |
| C5 Union | 3 | 0.8019 | 0.5403 | 0.2340 | 0.2863 | 0.3050 | 0.2751 | 0.3414 |
| C6 Dedup | 3 | 0.8025 | 0.5434 | 0.2383 | 0.2843 | 0.3003 | 0.2743 | 0.3416 |

### BERT-large × MNLI k=10% (3 epochs, lr=3e-5, 3 splits 평균) — 완료

실패 run 2개 제외 (C5_union/split1, C6_dedup/split3 — 학습 불안정, 재실행 중)

| Condition | N | Dev | HANS | ANLI-r1 | ANLI-r2 | ANLI-r3 | ANLI avg | OOD avg |
|-----------|---|------|------|---------|---------|---------|----------|---------|
| C1 Full | 3 | 0.8057 | 0.5355 | 0.2313 | 0.2767 | 0.3036 | 0.2705 | 0.3368 |
| C2 Random | 3 | 0.8006 | 0.5265 | 0.2287 | 0.2883 | 0.2986 | 0.2719 | 0.3355 |
| C3 Self-easy | 3 | 0.8012 | 0.5322 | 0.2340 | 0.2883 | 0.3089 | 0.2771 | 0.3409 |
| C4 Intersect | 3 | 0.8074 | 0.5361 | 0.2293 | 0.2790 | 0.3050 | 0.2711 | 0.3374 |
| C5 Union | 2 | 0.7878 | 0.5372 | 0.2405 | 0.2845 | 0.3150 | 0.2800 | 0.3443 |
| C6 Dedup | 2 | 0.7901 | 0.5381 | 0.2360 | 0.2835 | 0.3100 | 0.2765 | 0.3419 |

### BERT-large × MNLI: k% 비교 요약

**BERT-large에서는 k=5%, 10% 모두 condition 간 차이가 매우 작음** (OOD avg 범위: ~0.33-0.34).
T5-XL k=30%에서는 C5_union이 명확히 우세했으나, BERT-large에서는 그런 차이가 보이지 않음.

가능한 원인:
1. BERT-large가 T5-XL보다 작아서 shortcut 의존도 자체가 다를 수 있음
2. k=5%/10%는 제거량이 적어 (8-18%) 효과가 미미
3. BERT-large의 학습 불안정성 (4/36 run 실패)이 결과 노이즈를 증가
...
### T5-XL (k=30%): 핵심 메시지 지지
- C5 Union이 OOD에서 가장 좋고, dev 하락 없음 → 소형 모델 합집합 easy 제거가 효과적
- C3 Self-easy 비효과적 → "자기 자신의 easy 제거"는 도움이 안 됨
- C6 Dedup은 기대 이하 → dedup 전략 자체가 shortcut 제거 효과를 희석할 수 있음

### BERT-large (k=5%, 10%): 약한 신호
- 전반적으로 condition 간 차이 미미
- C5/C6이 약간 높은 경향은 있으나 통계적으로 유의하지 않을 수 있음

### ARC: 데이터 부족으로 부적합
- 3.4K에서 50-67% 제거하면 학습 자체가 어려움
- 필터링 실험에는 충분한 규모의 데이터가 필요

### 남은 실험
- [ ] BERT-large × MNLI k=30% 완료 (C2, C3, C6_split1 미완)
- [ ] BERT-large k=5%/10% 실패 run 재실행 (4 runs, 진행 중)
- [ ] 결과에 따라 Pythia-1B 실험 여부 결정
