"""ARC training for all model families: BERT, T5 v1.1, Pythia.
BERT: BertForMultipleChoice
T5 v1.1: Sentinel prompt + Adafactor + warmup
Pythia: A/B/C/D token prediction
"""
...
T5_CHOICE_LABELS = ["A", "B", "C", "D", "E"]  # verbalizer for T5
...
def load_arc_data():
    ...
    return train, val_overall, val_challenge
...
def train_and_eval(model, tokenizer, family, train_data, val_data, device, args):
    # DataLoader
    if family == "encoder":
    ...
    val_loader = DataLoader(ARCDataset(val_data), batch_size=16, shuffle=False, collate_fn=collate)
    ...
    # Label token IDs for T5 and Pythia
    t5_label_ids = None
    pythia_label_ids = None
    if family == "enc-dec":
        sentinel_id = tokenizer.encode("<extra_id_0>", add_special_tokens=False)[0]
        t5_label_ids = {l: tokenizer.encode(l, add_special_tokens=False)[0] for l in T5_CHOICE_LABELS}
    elif family == "decoder":
    ...
    for epoch in range(args.epochs):
        ...
        with torch.no_grad():
            for batch in val_loader:
                if family == "encoder":
                    ...
                    logits = out.logits[:, 1, :]
                    lid = [t5_label_ids[T5_CHOICE_LABELS[i]] for i in range(MAX_CHOICES)]
        ...
        print(f"  Epoch {epoch+1}: loss={avg_loss:.4f}, overall_acc={overall_acc:.4f}")
    ...
    return best_overall, best_epoch, epoch_results
...
def main():
    ...
    start = time.time()
    best_acc, best_epoch, epoch_results = train_and_eval(
        model, tokenizer, family, train_data, val_overall, device, args)
    elapsed = time.time() - start

    print(f"Best: overall_acc={best_acc:.4f} (epoch {best_epoch}), time={elapsed:.1f}s")
