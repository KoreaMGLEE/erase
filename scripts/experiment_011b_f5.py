"""Plan 011 Experiment B: Continuous weight-based curation for T5-large (full FT).
...
# Use T5-large confidence as "large model" (Self)
MNLI_CONF = {
...
def compute_weights(all_ids, split_num, method, agg_type, scale_type):
    ...
    if scale_type == "rank":
        def to_rank(conf_dict, ids):
            ...
            return {k: i / len(vals) for i, (k, _) in enumerate(vals)}
    ...
    for idx in all_ids:
        ...
        agg_s = np.mean(s_vals) if agg_type == "mean" else max(s_vals)
        ...
        if method == "prod":
            ...
            # F5: (1 - S²) × (ε + L), ε=0.5
            w = (1.0 - agg_s ** 2) * (0.5 + conf_l)
        else:
            w = 1.0
        weights[idx] = max(w, 1e-6)
    return weights
...
def normalize_weights(weights):
    ...
    mean_w = vals.mean()
    return {k: v / mean_w for k, v in weights.items()} if mean_w > 0 else weights
...
def train_and_eval(train_examples, sample_weights, split_name, condition, device, lr, epochs=5):
    ...
    tokenizer = T5Tokenizer.from_pretrained("google/t5-v1_1-large", legacy=True)
    model = T5ForConditionalGeneration.from_pretrained("google/t5-v1_1-large", torch_dtype=torch.bfloat16).to(device)
    ...
    results = {"condition": condition, "split": split_name, "model": "t5-v1_1-large",
    ...
    results["weight_stats"] = {"mean": float(w_arr.mean()), "std": float(w_arr.std()),
                                "min": float(w_arr.min()), "max": float(w_arr.max())}
    return results


CONDITIONS = {
    "F5_mean_raw":  ("f5", "mean", "raw"),
    "F5_max_raw":   ("f5", "max",  "raw"),
}
...
def main():
    ...
    tag = args.tag if args.tag else ("norm" if args.normalize else f"lr{args.lr}")
    outdir = os.path.join(EXPB_DIR, f"results_t5large_{tag}")
    ...
    for split in args.splits:
        ...
        for cond in args.conditions:
            ...
            start = time.time()
            res = train_and_eval(train_examples, sample_weights, split, cond, device, lr=args.lr)
