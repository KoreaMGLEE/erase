"""Plan 009 Phase 2: BERT-large retraining on MNLI with 6 conditions.
Eval on MNLI-dev, HANS, ANLI.
"""
...
def load_hans():
    """Load HANS evaluation set from pre-downloaded file."""
...
def load_anli(round_tag):
    """Load ANLI dev set for a given round."""
...
def evaluate(model, tokenizer, examples, device, batch_size=64, num_labels=3):
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            if num_labels == 2:
                # HANS: 2-class (entailment vs non-entailment)
                # Map 3-class logits: entailment=0, neutral+contradiction → non-entailment
                logits_3 = out.logits
...
def train_and_eval(condition, split_name, args):
    ...
    # Load indices
    indices_path = os.path.join(PLAN9_DIR, "mnli", split_name, f"{condition}_indices.json")
    ...
    # Also load the split indices (which 30K subset)
    split_indices_path = f"/workspace/erase/outputs/plan0/data/{split_name}_indices.json"
    with open(split_indices_path) as f:
        split_indices = json.load(f)

    # indices are string keys from confidence. Map back to dataset indices.
    # The confidence keys are the original MNLI train indices (as strings).
    ...
    # Final ANLI eval (on best model = last epoch for simplicity)
    anli_results = {}
    for r in ["r1", "r2", "r3"]:
        try:
            anli_ex = load_anli(r)
...
def main():
    ...
    for condition in args.conditions:
        for split in args.splits:
            print(f"\n=== {condition} / {split} ===")
            outdir = os.path.join(PLAN9_DIR, "mnli", "results")
