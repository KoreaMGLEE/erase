"""Plan 011 Phase 3: BERT-large training with Detector Set B zones."""
...
PLAN11B_DIR = "/workspace/erase/outputs/plan11_detB"
PLAN9_DIR = "/workspace/erase/outputs/plan9"
CONDITIONS = ["B0_baseline", "B3_graded", "B5_binary"]
...
def train_and_eval(condition, split_name, args):
    ...
    with open(os.path.join(PLAN11B_DIR, split_name, f"{condition}_indices.json")) as f:
        indices = json.load(f)
    ...
    results = {"condition": condition, "split": split_name, "detector_set": "B", "epochs": []}
    ...
    if best_state:
        ...
        model.to(device)
        print(f"  Restored best epoch {best_epoch} (dev={best_dev:.4f})")
...
def main():
    ...
    outdir = os.path.join(PLAN11B_DIR, "results")
