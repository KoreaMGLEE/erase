"""Fig 2: w14 with taller heights (w10/w8.5 panel aspect ratios)."""
import json, os
import numpy as np
import matplotlib
matplotlib.use("Agg")
...
def draw(fig_w, fig_h, jaccard_res, diff_res, grade_res, random_bl, tag):
    """All use va font sizes (designed for 14 wide). Height changes panel aspect."""
    plt.rcParams.update({
        "font.family": "serif",
    ...
    print(f"  Saved: fig2_combined_{tag} ({fig_w}x{fig_h})")
    plt.close(fig)
...
if __name__ == "__main__":
    ...
    # va original: 14 x 3.2
    # w10 aspect: panels narrower -> equivalent to making va taller
    # 14 wide, height = 3.2 * (14/10) * (h_w10/3.2_scaled)
    # Simpler: w10 h2.6 panel aspect = 10/(3*2.6) = 1.28
    #          For 14 wide: h = 14/(3*1.28) = 3.65

    # w8.5 aspect: even narrower panels
    # w8.5 h2.8 panel aspect = 8.5/(3*2.8) = 1.01
    #          For 14 wide: h = 14/(3*1.01) = 4.62

    variants = [
        # w10-like aspect ratios (14 wide, taller)
        (14, 3.2, "va_orig"),       # original va
        (14, 3.6, "va_h3p6"),       # w10 h2.6 equivalent
        (14, 4.2, "va_h4p2"),       # w10 h3.0 equivalent
        # w8.5-like aspect ratios (14 wide, even taller)
        (14, 4.6, "va_h4p6"),       # w8.5 h2.8 equivalent
        (14, 5.0, "va_h5p0"),       # w8.5 h3.0 equivalent
    ]

    for w, h, tag in variants:
        draw(w, h, jaccard_res, diff_res, grade_res, random_bl, tag)
