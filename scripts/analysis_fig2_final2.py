"""Fig 2 final: thin lines, small markers, large labels (reference style)."""
import json, os
import numpy as np
import matplotlib
matplotlib.use("Agg")
...
def draw(fig_w, fig_h, lw, ms, shade_alpha, tag):
    plt.rcParams.update({
        "font.family": "serif",
    ...
    # Large text, thin lines — like the reference screenshot
    title_fs = 11
    label_fs = 10
    tick_fs = 9
    legend_fs = 8
    ...
    for ax, res, negate, ylabel, title in panels:
        for family in ["BERT", "T5 v1.1", "Pythia"]:
            ...
            ax.plot(xs, plot_ys, color=family_colors[family], marker=family_markers[family],
                    ...
                    markeredgecolor=family_colors[family], markeredgewidth=0.5,
        ...
        if not negate:
            ax.axhline(y=random_bl, color="gray", linestyle="--", linewidth=0.6, alpha=0.5)
            ax.text(0.95, 0.06, "random", fontsize=7, color="gray",
    ...
    axes[2].legend(handles, labels, fontsize=legend_fs, loc='lower right',
                   ...
                   handlelength=1.5, handletextpad=0.4, markerscale=1.0)
    ...
    print(f"  Saved: fig2_final_{tag} (lw={lw}, ms={ms}, shade={shade_alpha})")
    plt.close(fig)
...
if __name__ == "__main__":
    ...
    # Thin lines, small markers, light shade — reference style
    variants = [
        # (w, h, lw, ms, shade_alpha, tag)
        (8.5, 2.8, 1.0, 3.5, 0.08, "thin1"),    # thin line, small marker
        (8.5, 2.8, 1.2, 4.0, 0.06, "thin2"),    # slightly thicker
        (8.5, 2.8, 0.8, 3.0, 0.05, "thin3"),    # very thin, smallest markers
        (8.5, 3.0, 1.0, 3.5, 0.07, "thin4"),    # thin1 + taller
    ]

    for w, h, lw, ms, sa, tag in variants:
        draw(w, h, lw, ms, sa, tag)
